...
def _time_key(t: str) -> int:
    ...
    → 秒级整数，用于时间排序
    支持两种格式：
      1. 'YYYY-MM-DD HH:MM:SS'（或 ISO‑8601 变体）
...
def calc_time_token_stats(
    ...
    *,
    ...
    """
    返回：
      • time_stats_df : 每行一个原始 timestamp，含累计 token 分布统计量
      • final_stats   : 所有样本在最终时间点的累计 token 分布统计量 (dict)
    """
    ...
    times_sorted = sorted(all_times, key=_time_key)       # 仍按分钟排序，但保留原串
    ...
    # return time_stats_df, final_stats
    return final_stats
